...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="text",
        help="Path to the file with captions",
    )
    parser.add_argument(
        "--max_train_samples",
        type=int,
...
class ImageCaptionDataset(Dataset):
    def __init__(self, images_dir: str, captions_fp: str, tokenize_fn,
                 image_transforms=None):
        super().__init__()

        with open(captions_fp, 'r') as f:
            self.names_to_captions = json.load(f)
        self.imgs_paths = sorted(glob(osp.join(images_dir, '*.*g')))
    ...
    def __getitem__(self, idx):
        img_fp = self.imgs_paths[idx]
        img = Image.open(img_fp).convert('RGB')
        if self.image_transforms is not None:
            img = self.image_transforms(img)

        name = osp.basename(img_fp)
        caption = self.names_to_captions[name]
        input_ids = self.tokenize_fn(caption)
...
def main():
    ...
    train_dataset = ImageCaptionDataset(args.train_data_dir, args.captions_fp,
